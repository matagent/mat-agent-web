...
- **`agent/langchain_mcp_agent.py`** — `MatAgentMCP` class. Connects to the MCP server via SSE, loads tools through `MultiServerMCPClient`, wraps them (MCP returns `list[dict]` but LangChain needs strings), creates a LangChain agent with `ChatOpenAI`. Supports `deepseek-chat`, `deepseek-reasoner`, and `glm-5` models. Provides sync (`chat`) and async (`chat_stream`) interfaces.
